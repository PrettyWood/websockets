...
- ``CONNECTING -> OPEN``: in
  ...
  or TLS connection is established;
- ``OPEN -> CLOSING``: in
...
- ``* -> CLOSED``: in
  ...
  called exactly once when the TCP or TLS connection is closed.
...
- :attr:`~protocol.WebSocketCommonProtocol.close_connection_task` runs
  ...
  the data transfer to terminate, then takes care of closing the TCP or TLS
  connection. It never exits with an exception and it is never cancelled.
  See :ref:`connection termination <connection-termination>` below.
...
``websockets`` contains a consistent implementation of both sides.
...
- calls :meth:`~server.WebSocketServerProtocol.process_request()` which may
  ...
  ability only makes sense on the server side;
- checks the HTTP request, negociates ``extensions`` and ``subprotocol``, and
  configures the protocol accordingly;
- builds a HTTP response based on the above as well as parameters passed to
  :meth:`~server.serve()`;
...
TCP or TLS connection. This is the proper way to fail the WebSocket connection
...
when the WebSocket connection is established, processes incoming frames and
makes messages available for the application.
...
:meth:`~protocol.WebSocketCommonProtocol.recv()` fetches the next message from
the messages queue, with some complexity added for handling termination
correctly.
...
:meth:`~protocol.WebSocketCommonProtocol.ping()` writes a ping frame, then
stores and returns a :class:`~asyncio.Future` which will be completed when a
matching pong frame is received.
...
from :attr:`~protocol.WebSocketCommonProtocol.transfer_data_task` because that
makes it easier to implement the timeout on the closing handshake. Cancelling
...
and thus failing to close the TCP connection.
...
the TCP or TLS connection is closed and the exception is re-raised or logged.

Once the WebSocket connection is established,
:attr:`~protocol.WebSocketCommonProtocol.transfer_data_task` and
:attr:`~protocol.WebSocketCommonProtocol.close_connection_task` must keep
running even if a coroutine that awaits them is cancelled. This means they
must be shielded from cancellation.
...
:meth:`~protocol.WebSocketCommonProtocol.ensure_open()`, which is called by
:meth:`~protocol.WebSocketCommonProtocol.send()`,
:meth:`~protocol.WebSocketCommonProtocol.ping()`, and
:meth:`~protocol.WebSocketCommonProtocol.pong()`, shields
:attr:`~protocol.WebSocketCommonProtocol.transfer_data_task`.
...
is cancelled. This is the only point where
:attr:`~protocol.WebSocketCommonProtocol.transfer_data_task` may be cancelled.

:meth:`~protocol.WebSocketCommonProtocol.close()` shields
